...
async function routeCall(evt){
	let url,pathName,request,pos,routeLead,routeObj;
	let callId=""+(nextCallIdx++);
	request=evt.request;
	url=new URL(request.url);
	pos=pathName.indexOf("/",2);//url should starts with: /+
	if(pos>0){
		routeLead=pathName.substring(2,pos);
	}
	routeObj=routes[routeLead];
	if(Array.isArray(routeObj)){
		let path,stub,len,maxLen=0,maxObj=null;
		for(stub of routeObj){
			path=stub.path;
			if(pathName.indexOf(path)===2){
				len=path.length;
				if(len>=maxLen){
					maxLen=len;
					maxObj=stub;
	...
	return new Promise((resolve,reject)=>{
		...
		if(routeObj) {
			...
			routeObj.client.postMessage({ msg: "RouteCall", callId: callId, method: request.method, url: request.url });
		}else{
...
self.addEventListener('message', function(event) {
	...
	switch(msg){
		case "Watch":
			...
			watchClients.push({client:event.source,id:watchIdName});
			break;
		case "Unwatch":{
			let clientId,stub;
